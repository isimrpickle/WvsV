...
string** map_create() {
    ...
    /*for (auto i = enemies.begin(); i != enemies.end(); i++) {
        graphics* part_one = *i;
        for (auto p = enemies.begin() + 1; p != enemies.end(); p++) {
            graphics* part_two = *p;
        }
        part_one->set_power(rand() % 4 + 1);
    }*/
    for (auto i = enemies.begin(); i != enemies.end(); i++) {
        graphics* part_one = *i;

        cout << part_one->getpower() << "\n" << endl;
    }
...
void run_away(graphics* graphic, string** array) {
    int x_before = graphic->get_x();
    int y_before = graphic->get_y();

    if (graphic->get_type() == WEREWOLF) {
        for (int i = 1; i <= 4; i++) {
            move_update(array, graphic, i);
            if (graphic->get_x() != x_before || graphic->get_y() != y_before)
                break;
        }
    }
    else {
